...
class LLMTranslator(BaseTranslator):
    ...
    def _prepare(self, translate_data_list: List[SubtitleProcessData]) -> None:
        ...
        self._credentials = get_llm_credentials()
    ...
    def _request(self, messages):
        """S4 requests own their socket and captured credentials; cancellation closes it."""
        if not self.is_running:
            raise RuntimeError("Translation cancelled.")
        if self.conversation_snapshot is None or not self.conversation_snapshot.context.enabled:
            return call_llm(messages=messages, model=self.model)
        credentials = self._credentials
        if credentials is None or not credentials.is_complete:
            raise ValueError("LLM credentials are not configured.")

        async def request():
            async with openai.AsyncOpenAI(api_key=credentials.api_key, base_url=credentials.base_url,
                                          max_retries=0, timeout=120,
                                          http_client=openai.DefaultAsyncHttpxClient(
                                              follow_redirects=False, timeout=120)) as client:
                task = asyncio.create_task(client.chat.completions.create(model=self.model, messages=messages))
                try:
        ...
        try:
            ...
            raise RuntimeError(f"Context translation HTTP {exc.status_code}; review or retry explicitly.") from None
        except (openai.APIConnectionError, TimeoutError):
            raise RuntimeError("Context translation timed out or lost connection; retry explicitly.") from None
