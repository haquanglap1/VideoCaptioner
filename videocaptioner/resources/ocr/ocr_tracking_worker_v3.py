"""Version 3 single-line visual tracking companion for the pinned offline OCR worker.
...
def foreground_boxes(boxes, width, height, anchor):
    crossing = [b for b in boxes if min(p[1] for p in b) <= anchor * height <= max(p[1] for p in b)
                ...
                and abs((min(p[1] for p in b) + max(p[1] for p in b)) / 2 - anchor * height)
                <= .35 * (max(p[1] for p in b) - min(p[1] for p in b))
    ...
    line_center = (max(p[1] for p in primary) + min(p[1] for p in primary)) / 2
    selected = [b for b in crossing if .75 * line_height <= max(p[1] for p in b) - min(p[1] for p in b)
                <= 1.35 * line_height
                and abs((max(p[1] for p in b) + min(p[1] for p in b)) / 2 - line_center) <= .25 * line_height]
    return selected
...
class CharacterTracker:
    def __init__(self, root, engine):
        ...
        self.previous = None
        self.previous_punctuation = None
        self.geometry = None
        self.anchor = None
        self.batches = 0
    ...
    def observe(self, raw, width, height, anchor, known_boxes, request_id):
        ...
        _, own_classes, own_scores, own_valid = own_features
        glyphs = own_valid & (own_classes != 0) & (own_classes != self.space) & (own_scores >= .9)
        # CTC blank/weak scene features need independent ink evidence. Standalone
        # punctuation often has no confident CTC label and must remain observable.
        punctuation = self.punctuation_mask(gray, support) if not np.any(glyphs) else None
        if not np.any(glyphs) and punctuation is None:
            changed = self.previous is not None
            self.previous = None
            return {"present": False, "changed": changed, "uncertain": False, "quality": 0.0}
        ...
        if previous_state is not None:
            ...
            if punctuation is not None and self.previous_punctuation is not None:
                # Blank CTC cannot bracket punctuation-only edits. Compare the observed
                # ink directly, including single-frame changes; never merge by duration.
                union = int(np.count_nonzero(punctuation | self.previous_punctuation))
                ink_change = int(np.count_nonzero(punctuation ^ self.previous_punctuation)) / max(1, union)
                changed = changed or ink_change > .2
                uncertain = (uncertain or ink_change > 0) and not changed
        self.previous = gray, support, ranges
        self.previous_punctuation = punctuation
    ...
    def punctuation_mask(self, gray, support):
        """Retain compact, aligned marks without interpreting them as recognized text."""
        np, cv2 = self.np, self.cv2
        ys, _ = np.where(support)
        line_height = int(ys.max() - ys.min() + 1)
        mode = int(np.bincount(gray[support], minlength=256).argmax())
        values = gray.astype('float32') - mode
        for polarity in (1, -1):
            contrast = float((values[support] * polarity).max())
            if contrast < 4:
                continue
            mask = np.uint8(support & (values * polarity >= max(4, contrast * .5)))
            count, _, stats, centers = cv2.connectedComponentsWithStats(mask, 8)
            components = [(s, c) for s, c in zip(stats[1:count], centers[1:count]) if s[4] >= 3]
            if not 1 <= len(components) <= 12:
                continue
            if any(not (1 <= s[2] <= .35 * line_height and 1 <= s[3] <= .35 * line_height
                        and .35 <= s[2] / s[3] <= 2.5 and s[4] >= .25 * s[2] * s[3])
                   for s, _ in components):
                continue
            ordered = sorted(components, key=lambda row: row[1][0])
            if (max(c[1] for _, c in ordered) - min(c[1] for _, c in ordered) <= .2 * line_height
                    and all(b[1][0] - a[1][0] <= line_height for a, b in zip(ordered, ordered[1:]))):
