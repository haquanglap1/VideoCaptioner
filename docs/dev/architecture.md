...
videocaptioner/core/entities.py   Entity dùng chung: TranscribeConfig, SubtitleConfig, *Task, enum
...
videocaptioner/core/subtitle/     Style, ASS/rounded renderer, editing (thao tác bảng phụ đề)
...
videocaptioner/core/editor/       Domain Video Editor: models, commands, project_store, media, voice
videocaptioner/core/llm/          Client OpenAI-compatible, credentials, context, request logger
...
`adapters.py` và `voice.py` (nối với dubbing/TTS). `ui/view/video_editor_interface.py` là page
