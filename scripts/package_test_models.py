...
def append_ocr_payload(target: Path, incoming: Path, *, component: str = "ocr") -> None:
    ...
    if component not in ("ocr", "ocr-v6-medium"):
        raise ValueError("Unknown OCR component")
    ...
    for root in (target, incoming):
        ...
        if not owner.is_file() or json.loads(owner.read_text(encoding="utf-8")).get("schema") != SCHEMA:
            raise ValueError("Model collection has no valid staging owner")
    ...
    if (manifest.get("schema") != SCHEMA or ocr.get("schema") != SCHEMA
            or ocr.get("components") != [component] or component in manifest.get("components", [])
            or (target / component).exists()):
    ...
    for name, expected in incoming_files.items():
        path = Path(name)
        if (not name.startswith(component + "/") or path.is_absolute() or ".." in path.parts
                or ":" in name or "\\" in name or name in manifest["files"]):
            raise ValueError("OCR inventory escapes its component")
        source = incoming / path
        if (not source.resolve().is_relative_to(incoming / component) or not source.is_file()
    ...
    actual = {p.relative_to(incoming).as_posix() for p in (incoming / component).rglob("*") if p.is_file()}
    if actual != set(incoming_files):
        raise ValueError("OCR component contains unlisted files")
    copy_payload(incoming / component, target / component)
    for name, expected in incoming_files.items():
